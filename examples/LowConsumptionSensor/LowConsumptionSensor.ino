/*
 * ChronosPower v2.5 - Пример IoT датчика с низким энергопотреблением
 * 
...
void optimizePeripherals() {
    ...
    CPower().control(Peripheral::UART_2, false);
    CPower().control(Peripheral::TIMER_1_16BIT_GENERAL, false);
    CPower().control(Peripheral::DAC_0_8BIT, false);
    CPower().control(Peripheral::DAC_1_12BIT, false);
    
    // Оставляем только необходимые
    CPower().control(Peripheral::ADC_0_12BIT, true);
    CPower().control(Peripheral::RTC_INTERNAL, true);
    CPower().control(Peripheral::UART_0, true); // Для Serial
...
void transmitData() {
    ...
    CPower().control(Peripheral::WIFI_MAC_2_4GHZ, true);
...
void showEnergyStats() {
    ...
    Serial.println("Среднее: " + String(energy.averageConsumptionMicroAmps) + " мкА");
...
void enterDeepSleep() {
    ...
    WakeupPolicy policy;
    policy.rtcTimeoutSeconds = DEEP_SLEEP_DURATION_MS / 1000;
    
    // Дополнительные источники пробуждения для экстренных случаев
    policy.extInterruptPinMask = (1ULL << 0); // Кнопка на пине 0
...
String wakeupReasonToString(WakeupReason reason) {
    switch (reason) {
        case WakeupReason::RTC_ALARM_SECONDS: return "Таймер";
        case WakeupReason::EXTERNAL_PIN_0: return "Внешний пин";
