...
// Важные (включены при нормальном питании)
...
**186. Очистка флага причины пробуждения**
...
**296. STM32: Использование LSE для RTC в режиме Standby**
...
void setup() {
  ...
  Serial.println("ChronosPower v2.5 инициализирован");
}
...
void loop() {
  ...
  // Интеллектуальный сон
  ChronosPower::WakeupPolicy policy;
  policy.rtc.timeoutSeconds = 60; // 1 минута
  policy.enableIntelligentWakeup = true;
  
  CPower().sleep(policy);
  
  // Анализ причины пробуждения
  WakeupReason reason = CPower().getWakeupReason();
  Serial.println("Пробуждение: " + String((int)reason));
}

void performMainTask() {
  // Ваша основная логика здесь
  Serial.println("Выполнение основной задачи...");
  delay(1000);
}
```

ChronosPower v2.5 - это библиотека управления питанием для встраиваемых систем нового поколения с возможностями искусственного интеллекта и машинного обучения.
